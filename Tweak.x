...
+ (void)filterChangedHandler:(UIButton *)sender {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        [btn setTitleColor:[UIColor lightGrayColor] forState:stream_id ? [UIColor lightGrayColor] : [UIColor lightGrayColor]];
    }
    [sender setTitleColor:[UIColor greenColor] forState:NSControlStateNormal];
}
...
+ (void)dumpKeychainItems {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSMutableDictionary *query = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                      ...
                                      (__bridge id)kSecBooleanTrue, (__bridge id)kSecReturnAttributes,
                                      (__bridge id)kSecBooleanTrue, (__bridge id)kSecReturnData, nil];
        ...
        if (status == errSecSuccess) {
            ...
            for (NSDictionary *item in items) {
                ...
                NSString *payload = @"";
                if (data) {
                    payload = [[NSString alloc] initWithData:data encoding:UTF8StringEncoding];
                    if (!payload) {
...
- (void)startLoading {
    ...
    @try {
        ...
        if ([urlStr containsString:@"buy.itunes.apple.com/verifyReceipt"]) {
            ...
            if (bodyData) {
                NSString *bodyString = [[NSString alloc] initWithData:bodyData encoding:UTF8StringEncoding];
        ...
        if (c_key_buf && c_hex_str) {
            ...
            @try {
                ...
                if (targetBuffer && targetLength > 0) {
                    dataStr = [[NSString alloc] initWithData:[NSData dataWithBytes:targetBuffer length:targetLength] encoding:UTF8StringEncoding];
                    if (!dataStr) {
